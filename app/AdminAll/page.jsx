...
export default function AdminAllDataPage() {
  ...
  const handleExport = async (type) => {
    try {
      const response = await fetch(`/api/admin/export?tab=${type}`);
    ...
    <div className="min-h-screen bg-gradient-to-br from-gray-50 to-gray-100">
      ...
      <div className="bg-gradient-to-r from-green-600 to-green-700 text-white shadow-lg">
        <div className="container mx-auto px-6 py-6">
          <div className="flex items-center justify-between">
            <div>
              <h1 className="text-3xl font-bold">Admin Dashboard - All Data</h1>
                ...
                <tbody className="divide-y divide-gray-200">
                  {filteredUsers.slice(0, 5).map((user) => (
                    <tr key={user.id} className="hover:bg-gray-50">
                      ...
                      <td className="px-6 py-4 text-gray-600">{user.orders || 0}</td>
                      <td className="px-6 py-4">
                        <div className="flex gap-2">
                ...
                <tbody className="divide-y divide-gray-200">
                  {filteredOrders.slice(0, 5).map((order) => (
                    <tr key={order.id} className="hover:bg-gray-50">
                      ...
                      <td className="px-6 py-4 text-gray-800 font-semibold">₹{order.amount?.toLocaleString()}</td>
                      <td className="px-6 py-4">
                        <select
                          value={order.status}
                ...
                <tbody className="divide-y divide-gray-200">
                  {filteredProducts.slice(0, 5).map((product) => (
                    <tr key={product.id} className="hover:bg-gray-50">
                      ...
                      <td className="px-6 py-4 text-gray-600">{product.stock} kg</td>
              ...
              <table className="w-full">
                ...
                <tbody className="divide-y divide-gray-200">
                  ...
                  {filteredServices.length > 5 && (
                    <tr>
                      <td colSpan="6" className="px-6 py-4 text-center text-gray-500">
                        And {filteredServices.length - 5} more services...
                      </td>
                    </tr>
                  )}
                </tbody>
